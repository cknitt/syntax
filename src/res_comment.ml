type style = SingleLine | MultiLine
...
let toString t =
  Format.sprintf "(txt: %s\nstyle: %s\nlines: %d-%d)" t.txt
    (styleToString t.style) t.loc.loc_start.pos_lnum t.loc.loc_end.pos_lnum

let makeSingleLineComment ~loc txt =
  {txt; loc; style = SingleLine; prevTokEndPos = Lexing.dummy_pos}

let makeMultiLineComment ~loc txt =
  {txt; loc; style = MultiLine; prevTokEndPos = Lexing.dummy_pos}
