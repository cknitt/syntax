...
let rec walkStructure s t comments =
  ...
  | s ->
    walkList
      ...
      ~walkNode:walkStructureItem s t comments
...
and walkStructureItem si t comments =
  ...
  | Pstr_recmodule moduleBindings ->
    walkList
      ...
      ~walkNode:walkModuleBinding moduleBindings t comments
...
and walkTypeExtension te t comments =
  ...
  walkList
    ...
    ~walkNode:walkExtConstr te.ptyext_constructors t rest
...
and walkSignature signature t comments =
  ...
  | _s ->
    walkList
      ...
      ~walkNode:walkSignatureItem signature t comments
...
and walkSignatureItem si t comments =
  ...
  | Psig_recmodule moduleDeclarations ->
    walkList
      ...
      ~walkNode:walkModuleDeclaration moduleDeclarations t comments
...
and walkValueBindings vbs t comments =
  walkList
    ...
    ~walkNode:walkValueBinding vbs t comments
...
and walkTypeDeclarations typeDeclarations t comments =
  walkList
    ...
    ~walkNode:walkTypeDeclaration typeDeclarations t comments
    ...
    | Ptype_record labelDeclarations ->
      let () =
        walkList
          ...
          ~walkNode:walkLabelDeclaration labelDeclarations t rest
      in
      []
...
and walkConstructorArguments args t comments =
  ...
  | Pcstr_tuple typexprs ->
    visitListButContinueWithRemainingComments
      ...
      ~walkNode:walkTypExpr ~newlineDelimited:false typexprs t comments
...
and walkExpr expr t comments =
  ...
  | Pexp_let (_recFlag, valueBindings, expr2) ->
    let comments =
      visitListButContinueWithRemainingComments
        ...
        comments
    in
    if isBlockExpr expr2 then walkExpr expr2 t comments
    else
  ...
  | Pexp_open (_override, longident, expr2) ->
    ...
    if isBlockExpr expr2 then walkExpr expr2 t rest
    else
  ...
  | Pexp_extension
    ...
    walkList
      ~getLoc:
        (fun ((longident, expr) :
               Longident.t Asttypes.loc * Parsetree.expression) ->
  ...
  | Pexp_assert expr | Pexp_lazy expr ->
    if isBlockExpr expr then walkExpr expr t comments
    else
  ...
  | Pexp_construct ({txt = Longident.Lident "::"}, _) ->
    walkList
      ...
      ~walkNode:walkExpr (collectListExprs [] expr) t comments
  ...
  | Pexp_array exprs | Pexp_tuple exprs ->
    walkList
      ~getLoc:(fun n -> n.Parsetree.pexp_loc)
      ~walkNode:walkExpr exprs t comments
  ...
  | Pexp_ifthenelse (ifExpr, thenExpr, elseExpr) -> (
    ...
    | None -> ()
    | Some expr ->
      if isBlockExpr expr || isIfThenElseExpr expr then walkExpr expr t comments
      else
        let leading, inside, trailing = partitionByLoc comments expr.pexp_loc in
        attach t.leading expr.pexp_loc leading;
        walkExpr expr t inside;
  ...
  | Pexp_for (pat, expr1, expr2, _, expr3) ->
    ...
    if isBlockExpr expr3 then walkExpr expr3 t rest
    else
  ...
  | Pexp_match (expr, cases) | Pexp_try (expr, cases) ->
    ...
    walkList
      ~getLoc:(fun n ->
        {n.Parsetree.pc_lhs.ppat_loc with loc_end = n.pc_rhs.pexp_loc.loc_end})
  ...
  | Pexp_fun (_, _, _, _) | Pexp_newtype _ -> (
    ...
    | Pexp_constraint (expr, typ)
      ...
      if isBlockExpr expr then walkExpr expr t comments
        ...
        attach t.trailing expr.pexp_loc trailing
    | _ ->
      if isBlockExpr returnExpr then walkExpr returnExpr t comments
      else
        let leading, inside, trailing =
...
and walkExprPararameter (_attrs, _argLbl, exprOpt, pattern) t comments =
  ...
  | Some expr ->
    ...
    if isBlockExpr expr then walkExpr expr t rest
    else
...
and walkCase case t comments =
  ...
  let comments =
    ...
    | None -> rest
  in
  if isBlockExpr case.pc_rhs then walkExpr case.pc_rhs t comments
  else
...
and walkModExpr modExpr t comments =
  ...
  | Pmod_apply (_callModExpr, _argModExpr) ->
    ...
    walkList
      ...
      ~walkNode:walkModExpr modExprs t comments
  | Pmod_functor _ -> (
...
and walkPattern pat t comments =
  ...
  | Ppat_array patterns ->
    walkList
      ...
      ~walkNode:walkPattern patterns t comments
  | Ppat_tuple patterns ->
    walkList
      ~getLoc:(fun n -> n.Parsetree.ppat_loc)
      ~walkNode:walkPattern patterns t comments
  ...
  | Ppat_record (recordRows, _) ->
    walkList
      ~getLoc:
        (fun ((longidentLoc, pattern) :
               Longident.t Asttypes.loc * Parsetree.pattern) ->
        {longidentLoc.loc with loc_end = pattern.Parsetree.ppat_loc.loc_end})
...
and walkTypExpr typ t comments =
  ...
  | Ptyp_tuple typexprs ->
    walkList
      ~getLoc:(fun n -> n.Parsetree.ptyp_loc)
      ~walkNode:walkTypExpr typexprs t comments
  ...
  | Ptyp_constr (longident, typexprs) ->
    ...
    walkList
      ~getLoc:(fun n -> n.Parsetree.ptyp_loc)
      ~walkNode:walkTypExpr typexprs t rest
  | Ptyp_arrow _ ->
...
and walkPackageConstraints packageConstraints t comments =
  walkList
    ~getLoc:(fun (longident, typexpr) ->
      {longident.Asttypes.loc with loc_end = typexpr.Parsetree.ptyp_loc.loc_end})
